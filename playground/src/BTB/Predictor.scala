...
class Predictor extends Module with BPUtils with PredParams{
    ...
    when(io.update.valid){
        when(bhtBank(bht_idx_u).valid){
            bhtBank(bht_idx_u).bhr := Cat(bhtBank(bht_idx_u).bhr(BHRLen - 2, 0), io.update.brTaken)
        }.otherwise{
            bhtBank(bht_idx_u).bhr := io.update.brTaken
            bhtBank(bht_idx_u).valid := true.B
        }

        when(phtBank(pht_idx_u).valid){
            phtBank(pht_idx_u).ctr := satUpdate(phtBank(pht_idx_u).ctr, 2, io.update.brTaken)
        }.otherwise{
            phtBank(pht_idx_u).ctr := Mux(io.update.brTaken, 2.U, 1.U)
            phtBank(pht_idx_u).valid := true.B
        }
    }
}