...
class My_Btb extends Module with BP_Utail{
    val io = IO(new Bundle{
        ...
        val updata = Input(new Btb_update_entry)
    })
//-----------------------读取指令的历史记录，明白跳转的目标-------------------------------//
...
//-------------------------------更新策略-----------------------------//
    ...
    bank_idx_u := get_idx(io.updata.update_pc,32,idx_len)
    tag_u := seg(io.updata.update_pc,2,tagsize)
    val hits_u =  (0 until Btb_ways).map(i => rEntry_u(i).tag === tag_u && rEntry_u(i).have)
    val lfsr = LFSR(log2Ceil(Btb_sets), io.updata.require)

    val w_way = UInt(1.W)
/*     val update_require_r = Reg(Bool())
    update_require_r := io.updata.require  //todo 大概需要完善 */
...
/*     when(io.updata.require){
        Btb_bank(bank_idx_u)(w_way).data := 
    }
 */
