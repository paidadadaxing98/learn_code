...
class MyDCache(val SIZE: Int, val LINE_SIZE: Int, val WAY_NUM: Int, val UseLRU: Boolean)
  ...
  val request_wdata_buffer      = RegInit(0.U(32.W))
  val request_wstrb_buffer      = RegInit(0.U(4.W))
  val request_size_buffer       = RegInit(0.U(2.W))
  ...
  cache_unbussy   := is_idle || is_lookup && cache_hit && normal_request || is_respond || is_replace && !need_read_axi
  ...
  io.rdata        := Mux(is_respond, ret_data_buffer(target_word_id), target_data(target_word_id))

  io.rd_req       := is_replace && need_read_axi 
  io.rd_type      := rd_type_buffer
  io.rd_addr      := rd_addr_buffer

  io.wr_req       := is_miss && need_write_axi
  io.wr_type      := wr_type_buffer
  io.wr_wstrb     := wr_wstrb_buffer
  io.wr_addr      := wr_addr_buffer
  io.wr_data      := wr_data_buffer

  
  
  val ret_data_merge          = Wire(Vec(4, UInt(8.W)))
  val target_data_merge       = Wire(Vec(4, UInt(8.W)))
  val ret_data_v              = io.ret_data.asTypeOf(Vec(4, UInt(8.W)))
  val target_data_v           = target_data(target_word_id).asTypeOf(Vec(4, UInt(8.W)))
  val request_wdata_buffer_v  = request_wdata_buffer.asTypeOf(Vec(4, UInt(8.W)))
  val write_hit_wdata         = Wire(Vec(LINE_WORD_NUM, UInt(32.W)))
  val data_bank_wdata         = Mux(is_lookup, write_hit_wdata, ret_data_buffer)
  for(i <- 0 until 4)
  {
    ret_data_merge(i)     := Mux(request_wstrb_buffer(i), request_wdata_buffer_v(i), ret_data_v(i))
    target_data_merge(i)  := Mux(request_wstrb_buffer(i), request_wdata_buffer_v(i), target_data_v(i))
  }

  for(i <- 0 until LINE_WORD_NUM)
  {
    write_hit_wdata(i) := Mux(target_word_id===i.asUInt, target_data_merge.asUInt, target_data(i))
  }
  ...
  for (i <- 0 until WAY_NUM) {
    ...
    DataBank(i).dina    := data_bank_wdata.asUInt
    DataBank(i).wea     := is_lookup && write_hit && way_hit(i) ||
                            is_respond && normal_request && (i.asUInt === replaced_way_buffer)
  ...
  for (i <- 0 until WAY_NUM) {
    ...
    // TagvBank(i).dina    := Mux(request_cacop_en_buffer, 0.U(21.W), Cat(request_tag_buffer, 1.U))
  ...
  when(is_idle) { // Idle, receive request
    ...
    // way_hit_buffer            := way_hit
    // read_data_buffer          := read_data
    // read_tag_buffer           := read_tag
    // read_valid_buffer         := read_valid
    ...
    { 
      ...
      need_write_axi        := (cacop_op_2 && cache_hit || cacop_op_1 || cacop_op_0 || normal_request) && 
                                replaced_dirty && replaced_valid || uncached_write
      // wr_req_buffer   := is_miss && need_write_axi
      wr_type_buffer  := Mux(uncached_en, Cat(0.U(1.W), request_size_buffer), "b100".U)
      wr_wstrb_buffer := Mux(uncached_en, request_wstrb_buffer, "b1111".U)
      wr_addr_buffer  := Mux(uncached_en, request_addr, replaced_addr)
      wr_data_buffer  := Mux(uncached_en, request_wdata_buffer, replaced_data)
    ...
    when(write_hit) { Dirty(request_index_buffer)(cache_hit_way) := true.B }

  }
  .elsewhen(is_miss) 
  { 
    when(io.wr_rdy || !need_write_axi) 
    { 
      need_read_axi       := uncached_read || normal_request
      // rd_req_buffer       := is_replace && need_read_axi 
      rd_type_buffer      := Mux(uncached_en, Cat(0.U(1.W), request_size_buffer), "b100".U)
      rd_addr_buffer      := Mux(uncached_en, request_addr, request_line_addr)

      main_state := CacheState.REPLACE 
    }
  ...
  .elsewhen(is_refill) 
  {
    when(io.ret_valid) 
    {
      ...
      ret_data_buffer(miss_data_ret_cnt) := 
                Mux(miss_data_ret_cnt===target_word_id && request_op_buffer, ret_data_merge.asUInt, io.ret_data)
      
      when(io.ret_last) 
      {
        //Updata dirty table
        //If the missed request is store operation, initalize the dirty value as true, other false.
        when(normal_request){ Dirty(replaced_index_buffer)(replaced_way_buffer) := request_op_buffer }
        //All data received  
        main_state := CacheState.RESPOND
      }
    }
  }
  .elsewhen(is_respond) 
