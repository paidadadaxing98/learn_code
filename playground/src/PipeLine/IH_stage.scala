...
//CsrUnit
  ...
  val only_a_issue=(decode_bits(0).excp_en||(b_conflict_a&& ~b_can_delay_issue)||
                    ...
                    decode_bits(0).inst_op===SDEF(OP_TLB)||decode_bits(1).inst_op===SDEF(OP_TLB)||
                    decode_bits(0).inst_op===SDEF(OP_ATOM)||decode_bits(1).inst_op===SDEF(OP_ATOM)||
                    decode_bits(0).inst_op===SDEF(OP_CSR)||decode_bits(1).inst_op===SDEF(OP_CSR)||
                    decode_bits(0).inst_op===SDEF(OP_CNT)||
                    decode_bits(0).inst_op===SDEF(OP_CACP)||decode_bits(1).inst_op===SDEF(OP_CACP)||
                    decode_bits(0).inst_op===SDEF(OP_IDLE)||decode_bits(1).inst_op===SDEF(OP_IDLE)||
                    ((decode_bits(0).inst_op===SDEF(OP_MUL)||decode_bits(0).inst_op===SDEF(OP_DIV))&&
                     (decode_bits(1).inst_op===SDEF(OP_LSU)||
                    ...
                    (decode_bits(0).inst_op===SDEF(OP_LSU)&&{(decode_bits(1).inst_op===SDEF(OP_LSU)||
                     decode_bits(1).inst_op===SDEF(OP_DIV) )}))
