...
//exu
  ...
  //Predictor - 替换为新的BTB模块
  // val ubtb = Module(new uBtb())  // 原来的模块
  val ubtb = Module(new BTB())     // 新的BTB模块，集成了两级预测器、LRU、命中计数器
  ...
  // 新BTB的命中/访问计数器输出（可选择性使用）
  // 如果需要监控BTB性能，可以将这些信号导出到顶层或用于调试
  // val btb_hit_count_0 = ubtb.io.hit_count_0
  // val btb_hit_count_1 = ubtb.io.hit_count_1
  // val btb_access_count_0 = ubtb.io.access_count_0
  // val btb_access_count_1 = ubtb.io.access_count_1
